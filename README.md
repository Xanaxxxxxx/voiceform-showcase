# Voiceform

> Public Showcase

Language: **English** | [繁體中文](README.zh-TW.md) | [简体中文](README.zh-CN.md)

<p align="center">
  <img src="Media/brand/voiceform-icon.png" alt="Voiceform icon" width="164" />
</p>

![Platform: iOS 26+](https://img.shields.io/badge/Platform-iOS%2026%2B-111827?style=flat-square)
![Runtime: Core ML](https://img.shields.io/badge/Runtime-Core_ML-2563eb?style=flat-square)
![TestFlight: Internal Beta](https://img.shields.io/badge/TestFlight-Internal_Beta-2563eb?style=flat-square)
![License: Proprietary](https://img.shields.io/badge/License-Proprietary-dc2626?style=flat-square)

Voiceform is an on-device voice mirror for real-time pitch, resonance, thinness, and voice exploration.

A calm, supportive iPhone experience for people shaping a voice that feels like their own.

## App Store Connect

For TestFlight and App Store review:

- [Voiceform Privacy Policy](privacy.md)
- [Voiceform Support](support.md)

## Overview

- on-device voice practice experience on iPhone
- real-time feedback across pitch, resonance, thinness, and articulation surfaces
- local-first design for private voice exploration
- no source code or internal implementation details in this public showcase

## Current Status

| Area | Status |
|---|---|
| iPhone app | Active development |
| TestFlight | Internal beta |
| Android | Planned |

## Product Lens

Voiceform is intended to feel supportive rather than judgmental, precise without becoming clinical, and expressive without losing technical credibility.

The core loop is simple: speak, see live feedback, adjust, and try again when it feels useful.

## Contributors

<p align="left">
  <a href="https://github.com/Xanaxxxxxx">
    <img src="https://github.com/Xanaxxxxxx.png?size=96" width="64" height="64" alt="Xana" style="border-radius:50%; object-fit:cover;" />
  </a>
  <a href="https://github.com/antarfrica">
    <img src="https://github.com/antarfrica.png?size=96" width="64" height="64" alt="Jeff" style="border-radius:50%; object-fit:cover;" />
  </a>
</p>

| Contributor | Role | Contact |
|---|---|---|
| [Xana](https://github.com/Xanaxxxxxx) | Lead Developer, HCI & iOS Engineering | `gliding_corers0h@icloud.com` |
| [Jeff](https://github.com/antarfrica) | Research Lead, ML Systems & Voice Science | [GitHub](https://github.com/antarfrica) |
