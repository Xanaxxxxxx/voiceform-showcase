# Voiceform

> Public Showcase

Language: [English](README.md) | **繁體中文** | [简体中文](README.zh-CN.md)

<p align="center">
  <img src="Media/brand/voiceform-icon.png" alt="Voiceform icon" width="164" />
</p>

![Platform: iOS 26+](https://img.shields.io/badge/Platform-iOS%2026%2B-111827?style=flat-square)
![Runtime: Core ML](https://img.shields.io/badge/Runtime-Core_ML-2563eb?style=flat-square)
![TestFlight: Internal Beta](https://img.shields.io/badge/TestFlight-Internal_Beta-2563eb?style=flat-square)
![License: Proprietary](https://img.shields.io/badge/License-Proprietary-dc2626?style=flat-square)

Voiceform 是一款以 iPhone 為核心、在裝置端運行的聲音鏡子，聚焦於 pitch、resonance、thinness 與 voice exploration 的即時回饋。

它提供平靜、支持性的 iPhone 體驗，幫助使用者探索更貼近自己的聲音。

## App Store Connect

供 TestFlight 與 App Store 審核使用：

- [Voiceform Privacy Policy](privacy.md)
- [Voiceform Support](support.md)

## 概覽

- iPhone 裝置端聲音練習體驗
- 即時回饋涵蓋 pitch、resonance、thinness 與 articulation surfaces
- 以 local-first 為核心的私人聲音探索
- 此公開展示倉庫不包含產品源碼或內部實作細節

## 目前狀態

| 項目 | 狀態 |
|---|---|
| iPhone app | 持續開發中 |
| TestFlight | 內部 beta |
| Android | 規劃中 |
| 公開倉庫範圍 | 展示用途，不是完整產品鏡像 |

## 產品方向

Voiceform 應該感覺支持而非審判，精準但不臨床化，具有表達感但仍保持技術可信度。

核心循環很簡單：說話、看見即時回饋、調整，並在覺得有幫助時再試一次。

## 貢獻者

<p align="left">
  <a href="https://github.com/Xanaxxxxxx">
    <img src="https://github.com/Xanaxxxxxx.png?size=96" width="64" height="64" alt="Xana" style="border-radius:50%; object-fit:cover;" />
  </a>
  <a href="https://github.com/antarfrica">
    <img src="https://github.com/antarfrica.png?size=96" width="64" height="64" alt="Fan Lok Wai" style="border-radius:50%; object-fit:cover;" />
  </a>
</p>

| 貢獻者 | 角色 | 聯絡方式 |
|---|---|---|
| [Xana](https://github.com/Xanaxxxxxx) | Lead Developer, HCI & iOS Engineering | `gliding_corers0h@icloud.com` |
| [Fan Lok Wai](https://github.com/antarfrica) | Research Lead, ML Systems & Voice Science | [GitHub](https://github.com/antarfrica) |
